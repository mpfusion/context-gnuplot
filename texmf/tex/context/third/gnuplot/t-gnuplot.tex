...
%D create control sequence \type{\????gnuplot}
...
%D MPinstance
%D
%D To prevent interference definition of an individual MP instance for this module
\defineMPinstance
	[\????gnuplot]
	[\s!format=metafun,
	 \c!method=\s!double]
...
\def\resetGNUPLOTgraphics#1%
	{\doloop
		{\doifMPgraphicelse{\????gnuplot::gpg:#1:\recurselevel}%
		...
		{\letbeundefined{\????gnuplot::@@MPGgpg:#1:\recurselevel}}%
		{\exitloop}}%
	\doloop
...
\def\doprocessGNUPLOTfile[#1][#2]%
  {\bgroup
      ...
      \long\def\dostartGNUPLOTgraphic[##1]##2\stopGNUPLOTgraphic
         {\startreusableMPgraphic{\????gnuplot::gpg:#1:##1}##2\stopreusableMPgraphic}%
      %------------------
      % for TikZ terminal
      %------------------
      \newcounter\tikzGNUPLOTnumber
